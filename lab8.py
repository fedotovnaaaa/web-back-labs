...
def create():
    ...
    if not title or not title.strip():
        return render_template('lab8/create_article.html', error="Тема статьи не может быть пустой")
...
def list():
    ...
    if not articles:
        return render_template('/lab8/articles.html', articles=articles, public_articles=public_articles, no_articles=True)

    return render_template('/lab8/articles.html', articles=articles, public_articles=public_articles, no_articles=False)
...
def edit_article(article_id):
    ...
    if not article:
        db_close(conn, cur)
        return render_template('lab8/error.html', error="Статья не найдена или у вас нет прав для ее редактирования")
    ...
    if not title or not title.strip():
        db_close(conn, cur)
        return render_template('lab8/edit_article.html', article=article, error="Тема статьи не может быть пустой")
    
    if not article_text or not article_text.strip():
        db_close(conn, cur)
        return render_template('lab8/edit_article.html', article=article, error="Текст статьи не может быть пустым")
...
@lab8.route('/lab8/profile', methods=['GET', 'POST'])
def profile():
    login = session.get('login')
    if not login:
        return redirect('/lab8/login')
    ...
    if request.method == 'GET':
        ...
        user = cur.fetchone()
        db_close(conn, cur)
        return render_template('lab8/profile.html', real_name=user['real_name'] if user else '')
    ...
    if not real_name:
        db_close(conn, cur)
        return render_template('lab8/profile.html', error="Имя не может быть пустым")
